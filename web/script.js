document.addEventListener('DOMContentLoaded', function() {
    ...
    const testOrderResult = document.getElementById('testOrderResult');
    ...
    testOrderBtn.addEventListener('click', function () {
        testOrderResult.innerHTML = '<span style="color: #f39c12;">⏳ Sending order...</span>';

        fetch(`${apiBaseUrl}/add_order`, {
            method: 'POST',
            headers: {
            ...
            .then(response => {
                if (response.ok) {
                    return response.json().then(data => {
                        testOrderResult.innerHTML = '<span style="color: #27ae60;">✅ Order created successfully! (UID: ' + data.order_uid + ')</span>';
                    });
                } else {
            ...
            .catch(error => {
                testOrderResult.innerHTML = '<span style="color: #e74c3c;">❌ Error: ' + error.message + '</span>';
            });
    });
});